        ...
        Ok(handle) => {
            match handle {
                ...
                _ => {
                    ...
                    Err(format!("Font {} is in memory, using built-in font", font_name))
                }
            }
        }
        Err(_) => {
...
fn generate_svg_without_text(
    ...
    use ::svg::node::element::{Circle, Group, Line, Path as SvgPath, Rectangle};
    use ::svg::Document;
...
pub fn export_pdf(
    ...
    // Generate SVG without text and collect text elements
    let (svg_string, text_elements) = generate_svg_without_text(tree, layout, painter, width, height);
    ...
    let pdf_svg = Svg::parse(&svg_string)
    ...
    for text_elem in &text_elements {
        let font = font_cache.get(&text_elem.font_name)
